...
## #87 — FIFO-style whole-SSTable TTL expiry for LSM (drop fully-expired segments without rewrite)

TTL reclamation on the LSM engine is compaction-only: reads filter expired records, but bytes are reclaimed solely when a merge rewrites them ([src/size_tiered.rs](src/size_tiered.rs), [src/leveled.rs](src/leveled.rs)). Observed on the Pi telemetry deployment (2026-06-10): 8× ~27MB `metrics_*.sst` over 8 days with a 24h collection TTL — ~6 segments contained only expired data, never reclaimed, because auto-compaction defaults to off and the `-cr` ratio trigger is a no-op on LSM (`LsmEngine::dead_bytes`/`total_bytes` return 0). For the telemetry workload — append-only, unique time-ordered keys, uniform per-collection TTL — even correct dead-bytes accounting wouldn't fire: nothing is overwritten or deleted, data only _expires_. Implement RocksDB-FIFO-style whole-segment expiry: track the **max `expiry_ms`** per SSTable (computed at flush/compaction time; persisted in a footer/sidecar, or rebuilt on load alongside the bloom/sparse index), and when `now_ms > max_expiry` for a segment, unlink the whole file — zero rewrite I/O. A segment with any no-TTL record (`expiry_ms` absent/0) is never eligible. Safety: dropping a segment that contains tombstones can resurrect shadowed values in _older_ segments — restrict dropping to oldest-first order (a segment is droppable only when every older overlapping segment is droppable too, mirroring the #82 overlap reasoning), which is the natural order for time-keyed telemetry anyway. Trigger the check on flush and/or in `compact_step` so it runs without `-cr`/`-cms` configured. Interacts with #82 (conditional tombstone GC) and supersedes part of the deferred background-sweeper idea (#79 in the TTL design Future Work). Out of scope: per-record partial expiry (stays compaction-only), KV engine.
